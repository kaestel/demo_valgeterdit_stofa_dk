...
Util.Objects["page"] = new function() {
	this.init = function(page) {
		...
		page.resized = function() {
			page.browser_h = window.innerHeight; 
...
Util.Objects["IntroScene"] = new function() {
	this.init = function(node) {
		node.intro_scene = u.createPIXIApp({"classname":"intro_scene", "webgl":true});
		...
		node.intro_scene.ready = function() {
			...
			this.introIn = function(){
				var tSpeed = 3;
				Tween.to(this.introContainer,tSpeed, {y:250, ease:Elastic.easeOut.config(0.15)});
			}
		...
		this.assets.prize_tv.x = 363*0.64;
		this.assets.prize_tv.y = 250*0.64;
		...
		this.assets.prize_tablet.xPos = (363*0.64) + (this.assets.prize_tablet.width/2);
		this.assets.prize_tablet.yPos = (250*0.64) + (this.assets.prize_tablet.height/2);
		...
		this.assets.prize_nespresso.xPos = (590*0.64) + (this.assets.prize_nespresso.width/2);
		this.assets.prize_nespresso.yPos = (325*0.64) + (this.assets.prize_nespresso.height/2);
		...
		this.assets.prize_mac_dropshadow.x = (500*0.64) + (this.assets.prize_mac_dropshadow.width/2);
		this.assets.prize_mac_dropshadow.y = (316*0.64) + (this.assets.prize_mac_dropshadow.height/2);
		...
		this.assets.prize_mac.xPos = (500*0.64) + (this.assets.prize_mac.width/2);
		this.assets.prize_mac.yPos = (316*0.64) + (this.assets.prize_mac.height/2);
		...
		this.assets.prize_gavekort_shadow.x = (590*0.64) + (this.assets.prize_gavekort_shadow.width/2);
		this.assets.prize_gavekort_shadow.y = (283*0.64) + (this.assets.prize_gavekort_shadow.height/2);
		...
		this.assets.prize_gavekort.xPos = (590*0.64) + (this.assets.prize_gavekort.width/2);
		this.assets.prize_gavekort.yPos = (290*0.64) + (this.assets.prize_gavekort.height/2);
		...
		this.assets.prize_appletv.xPos = (348*0.64) + (this.assets.prize_appletv.width/2);
		this.assets.prize_appletv.yPos = (435*0.64) + (this.assets.prize_appletv.height/2);
        ...
        this.hoverDiff = 5;
        this.introContainer.x = ((this.renderer.width/2) - (this.introContainer.width/2))-13;
...
Util.Objects["StartScene"] = new function() {
	this.init = function(node) {
		node.start_scene = u.createPIXIApp({"classname":"start_scene", "webgl":true});
		...
		node.start_scene.ready = function() {
			...
			this.addBtn = function(){
				...
				this.startBtnContainer.y = 720;
				this.screenContainer.addChild(this.startBtnContainer);
				this.startBtnContainer.addChild(this.assets.startbtn_glow);
				this.assets.startbtn_glow.alpha = 0.7;
		        ...
		        this.assets.startbtn_hitarea.x = 95;
		        this.assets.startbtn_hitarea.y = 77;
			...
			this.startBtnOut = function(){
				this.doSparkLoop = false;
				Tween.delayedCall(.4, this.vedOut.bind(this));
				var tSpeed = .2;
				Tween.to(this.assets.startbtn_glow,tSpeed,{alpha:0, ease:Quad.easeIn});
				TweenLite.fromTo(this.assets.startbtn_rollover, .3, {alpha:0},{
					delay:0,
		            alpha: 1,
		            ease: RoughEase.ease.config({
		                template: Power0.easeNone,
			...
			this.vedHover = function(){
				if(this.isHovering) {
					this.hoverDiff*=-1;
					var tSpeed = 3;
					Tween.to(this.vedContainer,tSpeed,{y:this.vedStartY + this.hoverDiff, ease:Quad.easeInOut});
					Tween.delayedCall(tSpeed, this.vedHover.bind(this));
				}
			}
			this.vedIn = function(){
				var tSpeed = 5;
				Tween.to(this.screenContainer,tSpeed, {y:this.startY, ease:Elastic.easeOut.config(0.15)});
				Tween.delayedCall(tSpeed,function(){this.isHovering = true;this.vedHover();}.bind(this));
			}
			this.vedOut = function(){
				this.isHovering = false;
				var tSpeed = 0.5;
				Tween.to(this.screenContainer,tSpeed, {y:this.screenContainer.height*-1, ease:Quad.easeIn, onComplete:this.onStartBtnOut.bind(this)});
			}
			this.powerUp = function(){
				this.makeSplashBlast();
				TweenLite.to(this.vedPowered, 1.3, {
			...
			this.insertSparks = function(){
				...
				this.screenContainer.addChild(this.sparkContainer);
				this.sparks = [];
				var totalSparks = 5;
			...
			this.drawSpark = function() {
				var sparkObjTexture = PIXI.Texture.fromImage('/img/assets/smartphone/ved_spark.png');
		        var sparkObj = new PIXI.Sprite(sparkObjTexture);
		        sparkObj.anchor.set(0.5, 0.5);
		        sparkObj.scale.x = sparkObj.scale.y = u.random(70,100)/100;
		        this.stage.addChild(sparkObj);
		        this.sparks.push(sparkObj);
			}
			this.doSingleSpark = function(){
				if(this.doSparkLoop!=false){
					if(this.vedContainer.scale.x == 1){
			...
			this.screenContainer.addChild(this.vedContainer);
			this.startY = 0;
			this.vedContainer.y = 200;
			this.vedStartY = this.vedContainer.y;
			this.screenContainer.y = -800;
			this.screenContainer.addChild(this.assets.start_jan);
			this.screenContainer.addChild(this.vedContainer);
...
Util.Objects["GameScreen"] = new function() {
	this.init = function(node) {
		node.game_screen = u.createPIXIApp({"classname":"game_screen"});
		...
		node.game_screen.ready = function() {
			...
			this.setHeader = function(round) {
				...
				if(round !== false) {
					var texture = PIXI.Texture.fromImage('/img/assets/smartphone/' + round + '.png');
					this.header_asset = new PIXI.Sprite(texture);
					this.screenContainer.addChild(this.header_asset);
					this.header_asset.x = (this.renderer.width/2)-(310/2);
					this.header_asset.y = 150;
				}
			}
			this.screenContainer = new PIXI.Container();
			this.stage.addChild(this.screenContainer);
...
Util.Objects["GameAlreadyPlayed"] = new function() {
	this.init = function(node) {
		...
		node.game_already_played = u.createPIXIApp({"classname":"game_already_played"});
		...
		node.game_already_played.ready = function() {
			console.log("node.game_already_played.ready");
			this.addBtn = function(){
				...
				this.btnContainer.y = 770 + this.stageHeightDiff;
				this.screenContainer.addChild(this.btnContainer);
			...
			this.btnOut = function(){
				this.assets.btn_se_naeste_uges_praemie_rollover.alpha = 1;
			...
			this.assets.txt_already_played.y = 320;
...
Util.Objects["GameSelector"] = new function() {
	this.init = function(node, state) {
		node.game_selector = u.createPIXIApp({"classname":"game_selector"});
		...
		node.game_selector.ready = function() {
			...
			this.setupBtn = function(roundBtnContainer,round){
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/round_back' + round +'.png');
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/round_btn.png');
				asset = new PIXI.Sprite(texture);
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/round_btn_rollover.png');
				asset = new PIXI.Sprite(texture);
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/round_hitarea.png');
				asset = new PIXI.Sprite(texture);
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/round_week'+ round + '.png');
				asset = new PIXI.Sprite(texture);
			...
			this._onBrickSelected = function(){
				...
				for(var i=1;i<=6;i++){
					...
					if(i == this.app.node.scene.current_round_index+1){
						btnObj.rollover.alpha = 1;
						TweenLite.fromTo(btnObj, .3, {alpha:1},{
							delay:.1,
				            alpha: 0,
				            ease: RoughEase.ease.config({
				                template: Power0.easeNone,
						...
						Tween.to(btnObj,0.1,{delay:u.random(0,20)/100,alpha:0});
					}
				}
			...
			this.btn5.x = btnStackX + (btnWidth*0);
			this.btn5.y = btnStackY + (btnHeight*2);
			this.btn6 = this.createBtn(5);
			this.btn6.x = btnStackX +  btnMargin + (btnWidth*1);
			this.btn6.y = btnStackY + (btnHeight*2);
			this.btn3 = this.createBtn(2);
			this.btn3.x = btnStackX + (btnWidth*0);
			this.btn3.y = btnStackY + (btnHeight*1);
			this.btn4 = this.createBtn(3);
			this.btn4.x = btnStackX +  btnMargin + (btnWidth*1);
...
Util.Objects["GameBricks"] = new function() {
	this.init = function(node, state) {
		node.game_bricks = u.createPIXIApp({"classname":"game_bricks"});
		...
		node.game_bricks.ready = function() {
			...
			this.node.buttonUnderlay(false);
			this.createBrick = function(category, question) {
				...
				brickContainer.x = brickStackX + (brickWidth*category) + brickMargin;
			...
			this.setupActiveBrick = function(brickContainer){
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/categories_brick.png');
				asset = new PIXI.Sprite(texture);
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/categories_brick_rollover.png');
				asset = new PIXI.Sprite(texture);
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/categories_brick_'+brickContainer.question.stake+'.png');
				asset = new PIXI.Sprite(texture);
				...
				texture = PIXI.Texture.fromImage('/img/assets/smartphone/categories_brick_hitarea.png');
				asset = new PIXI.Sprite(texture);
			...
			this.setupInactiveBrick = function(brickContainer) {
				...
				var texture = PIXI.Texture.fromImage('/img/assets/smartphone/categories_brick_back_' + brickContainer.category_index +'_'+ brickContainer.question_index + '.png');
			...
			this._onBrickSelected = function(){
				...
				for(var i=1;i<=9;i++){
					...
					if(i == (this.app.node.scene.current_category_index * 3) + (this.app.node.scene.current_question_index+1)){
						brickObj.brick_rollover.alpha = 1;
						TweenLite.fromTo(brickObj, .3, {alpha:1},{
							delay:.1,
				            alpha: 0,
				            ease: RoughEase.ease.config({
				                template: Power0.easeNone,
						...
						Tween.to(brickObj,0.1,{delay:u.random(0,20)/100,alpha:0});
					}
				}
			...
			this.brick3 = this.createBrick(0, 2);
			this.brick6 = this.createBrick(1, 2);
			this.brick9 = this.createBrick(2, 2);
			this.brick2 = this.createBrick(0, 1);
			this.brick5 = this.createBrick(1, 1);
			this.brick8 = this.createBrick(2, 1);
			this.brick1 = this.createBrick(0, 0);
			this.brick4 = this.createBrick(1, 0);
			this.brick7 = this.createBrick(2, 0);
...
Util.Objects["GameQuestion"] = new function() {
	this.init = function(node) {
		node.game_question = u.createPIXIApp({"classname":"game_question"});
		...
		node.game_question.ready = function() {
			...
			if(this.node.scene.current_round_index !== false && this.node.scene.current_category_index !== false && this.node.scene.current_question_index !== false) {
				...
				this.setupBtn = function(answerBtnContainer,answerBtnNum){
					...
					texture = PIXI.Texture.fromImage('/img/assets/smartphone/answer_btn_'+ answerBtnNum + '.png');
					asset = new PIXI.Sprite(texture);
					...
					texture = PIXI.Texture.fromImage('/img/assets/smartphone/answer_rollover_'+ answerBtnNum + '.png');
					asset = new PIXI.Sprite(texture);
					...
					texture = PIXI.Texture.fromImage('/img/assets/smartphone/answer_hitarea.png');
					asset = new PIXI.Sprite(texture);
				...
				this.onBrickSelected = function(selectedCategory){
					for(var i=1;i<=4;i++){
						...
						if(i!=selectedCategory){
							...
							btnObj.answerBtn_rollover.alpha = 1;
							TweenLite.fromTo(btnObj, .3, {alpha:1},{
								delay:.1,
					            alpha: 0,
					            ease: RoughEase.ease.config({
					...
					Tween.delayedCall(.3, this.onAnswerBtnsOut.bind(this));
				}
				...
				this.btn4.x = btnStackX;
				this.btn4.y = btnStackY + (btnHeight*4);
				this.btn3 = this.createBtn(3);
				this.btn3.x = btnStackX;
				this.btn3.y = btnStackY + (btnHeight*3);
				this.btn2 = this.createBtn(2);
				this.btn2.x = btnStackX;
				this.btn2.y = btnStackY + (btnHeight*2);
				this.btn1 = this.createBtn(1);
				this.btn1.x = btnStackX;
				this.btn1.y = btnStackY + (btnHeight*1);
				...
				u.textscaler(this.node.div_question, {
					"min_width":320,
					"max_width":1024,
					"unit":"px",
					"h2":{
...
Util.Objects["GameSignupAlreadyPlayed"] = new function() {
	this.init = function(node) {
		u.bug("GameSignupAlreadyPlayed.init:" + u.nodeId(node));
		node.game_signup_already_played = u.createPIXIApp({"classname":"game_signup_already_played"});
		...
		node.game_signup_already_played.ready = function() {
			console.log("node.game_signup_already_played.ready");
			this.addBtn = function(){
				this.btnContainer = new PIXI.Container();
				this.screenContainer.addChild(this.btnContainer);
				this.btnContainer.pivot.set(this.assets.btn_se_naeste_uges_praemie.width/2,(this.assets.btn_se_naeste_uges_praemie.height/2)-16);
				this.btnContainer.x = this.renderer.width/2;
				this.btnContainer.y = 770;
				this.screenContainer.addChild(this.btnContainer);
				this.btnContainer.addChild(this.assets.btn_se_naeste_uges_praemie);
				this.btnContainer.addChild(this.assets.btn_se_naeste_uges_praemie_rollover);
				this.assets.btn_se_naeste_uges_praemie_rollover.alpha = 0;
		...
		node.game_signup_already_played.build = function() {
			u.bug("node.game_signup_already_played.build");
			this.node.game_screen.setHeader(false);
			this.showScreen();
		}
		node.game_signup_already_played._destroy = function() {
			u.bug("node.game_signup_already_played.destroy:" + onto);
			this.destroy(true);
	...
	// 
	this.build = function(node) {
		u.bug("GameSignupAlreadyPlayed.build");
...
Util.Objects["BtnPlayMore"] = new function() {
	this.init = function(node) {
		...
		node.btn_play_more.ready = function() {
			...
			this.btnOut = function(){
				var tSpeed = .2;
				this.assets.btn_spil_med_om_flere_lodder_rollover.alpha = 1;
				TweenLite.fromTo(this.btnContainer, .3, {alpha:1},{
					delay:.1,
		            alpha: 1,
		            ease: RoughEase.ease.config({
...
Util.Objects["BtnSignup"] = new function() {
	this.init = function(node) {
		node.btn_signup = u.createPIXIApp({"classname":"btn_signup", "height":185, "width":626});
		node.btn_signup.node = node;
		node.appendChild(node.btn_signup.renderer.view);
